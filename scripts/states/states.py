...
enclosing-cone aspect ratio with `csar`, prints the full ranking, and writes
...
`csar.solve(geom)` consumes them directly via `__geo_interface__` (the
MultiPolygon's exterior rings become the point set). `csar.plot_cone` then
...
def main():
    ...
    for name, geom in zip(gdf['name'], gdf.geometry):
        r = csar.solve(geom, max_outer=1000)
