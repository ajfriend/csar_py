...
each shapely geometry is fed straight to `csar.solve(geom)` — the geometry's
`__geo_interface__` turns its (multi)polygon rings into the point set. The plot
re-projects each ring with `csar.to_vec3(ring, geo='lonlat')` (GeoJSON axis
order). This collapses the original Zig-repo pipeline
(`gen_states.py → states.json → states.zig → states_aspect.json → plot`) into
a single script now that `csar.solve` is callable from Python.
