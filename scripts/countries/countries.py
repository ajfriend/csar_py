...
country's tightest enclosing-cone aspect ratio with `csar`, prints the full
...
`csar.solve(geom)` consumes them via `__geo_interface__`, and `csar.plot_cone`
...
spans ~38° of latitude) are slow to certify — so a few need more than csar's
...
def save_plot(name, geom, r):
    ax = csar.plot_cone(r, geom, title=f'{name} — AR {r.aspect_ratio:.3f}')
...
def main():
    ...
    for name, geom in zip(gdf[NAME_FIELD], gdf.geometry):
        try:
            r = csar.solve(geom, max_outer=MAX_OUTER)
